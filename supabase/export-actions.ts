...
import { istDateStr, monthBounds } from "@/lib/dates";
...
export async function buildPerformanceCsv(input?: { month?: string; divisionId?: string | null }) {
  const me = await getProfile();
  if (!me || (me.role !== "admin" && me.role !== "manager")) return { ok: false as const, error: "Not authorized" };

  const month = input?.month ?? istDateStr().slice(0, 7); // YYYY-MM
  ...
  return { ok: true as const, filename: `recruiter-performance-${month}${scope ? "-scoped" : ""}.csv`, csv: lines.join("\n") };
}
