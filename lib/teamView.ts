...
// The complete team dashboard payload (used by the manager/admin Team view AND the HR
// Performance tab, so they are identical). Read via service-role; callers must role-gate.
export async function buildTeamDashboard() {
  ...
  function buildData(divId: string | null, verifiedOnly: boolean) {
    ...
    const overallPie = funnel.filter((f: any) => f.count > 0).map((f: any) => ({ name: f.stage, value: f.count }));
  ...
  return { data: buildPayload(false), verified: buildPayload(true), today };
}
