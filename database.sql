...
-- ---------- ALLOCATIONS (persistent assignment of reqs to recruiters) ----------
-- An allocation is an ongoing assignment; it stays until removed (not per-day).
-- allocation_date records WHEN the assignment was made (informational).
create table allocations (
  ...
  created_at      timestamptz not null default now(),
  constraint allocations_req_recruiter_key unique (requirement_id, recruiter_id)
);
...
create table daily_activity (
  ...
  internal_submissions int not null default 0,       -- self-reported daily count
  client_submissions   int not null default 0,       -- self-reported daily count
  notes             text,
...
-- ---------- MULTI-DIVISION (a recruiter may belong to several divisions) ----------
-- profiles.division_id stays as the PRIMARY/home division (used for the division a
-- recruiter's own daily summary counts toward). profile_divisions is the FULL set.
...
-- ---------- PER-REQUIREMENT DAILY ACTIVITY (effort logged against a specific req) ----------
-- One row per recruiter / requirement / day. daily_activity (above) is kept as the
-- rolled-up per-day summary so badges + manager "logged today" cards keep working.
create table daily_activity_items (
  id                uuid primary key default gen_random_uuid(),
  recruiter_id      uuid not null references profiles(id),
  requirement_id    uuid not null references requirements(id) on delete cascade,
  division_id       uuid not null references divisions(id),
...
create index idx_dai_recruiter_date on daily_activity_items (recruiter_id, activity_date);
create index idx_dai_division_date  on daily_activity_items (division_id, activity_date);

-- ---------- DAILY METRICS (admin-configurable; the things recruiters log each day) ----------
-- Admins add / edit / delete / reorder these at runtime. Each metric chooses how it's
-- entered on the form (slider | stepper | chips | tally | dial).
create table daily_metrics (
  ...
  input_style  text not null default 'slider',
  soft_max     int  not null default 20,
  sort_order   int  not null default 0,
  is_active    boolean not null default true,
  created_at   timestamptz not null default now()
);
create index idx_daily_metrics_active on daily_metrics (is_active, sort_order);

-- ---------- DAILY ACTIVITY VALUES (one row per recruiter/requirement/day/metric) ----------
-- EAV so the metric list can change without a schema migration.
create table daily_activity_values (
  id             uuid primary key default gen_random_uuid(),
  recruiter_id   uuid not null references profiles(id),
  requirement_id uuid not null references requirements(id) on delete cascade,
  division_id    uuid not null references divisions(id),
...
create index idx_dav_recruiter_date on daily_activity_values (recruiter_id, activity_date);
create index idx_dav_division_date  on daily_activity_values (division_id, activity_date);
create index idx_dav_metric         on daily_activity_values (metric_id);

-- ---------- RECRUITER GOALS (per-month targets set by admin/manager) ----------
create table recruiter_goals (
  id                uuid primary key default gen_random_uuid(),
...
create index idx_recruiter_goals_recruiter on recruiter_goals (recruiter_id);
...
create table submissions (
  ...
  total_experience  numeric,                          -- years (e.g. 4.5)
  current_company   text,
  current_title     text,
  current_ctc       text,                             -- free-form (e.g. "12 LPA")
  expected_ctc      text,
  notice_period     text,
  source            text,                             -- LinkedIn / Naukri / Referral / Job board
  key_skills        text,
  resume_url        text,
  linkedin_url      text,
...
-- ---------- MESSAGES (manager/admin -> recruiter inbox; delivered in-app + email) ----------
...
-- ---------- BADGES (admin-configurable achievement definitions) ----------
create table badges (
  id            uuid primary key default gen_random_uuid(),
  code          text unique not null,
  name          text not null,
...
-- ---------- RECRUITER BADGES (awards; one row per recruiter/badge/period) ----------
...
create index idx_recruiter_badges_recruiter on recruiter_badges (recruiter_id);
...
create trigger trg_dav_updated
  before update on daily_activity_values
  for each row execute function set_updated_at();
...
insert into divisions (code, name) values
  ('DOM','Internal Hiring'), ('IND_IT','India IT'), ('US_IT','US IT');
...
-- Default daily metrics (admin-configurable; incl. internal & client submissions).
insert into daily_metrics (key, label, hint, color, icon, input_style, soft_max, sort_order) values
...
insert into app_settings (key, value, description) values
  ...
  ('leaderboard_weights', '{"submissions": 1, "closures": 5, "active_days": 2}',
   'Weights for the configurable leaderboard score.');

-- Achievement badge catalog (realistic for 15–25 submissions/month). Admin-configurable.
...
-- multi-division membership: authenticated read (allocation UI needs it), admin writes
...
-- Requirement creation is admin-only; managers may update/allocate but not create or delete.
create policy req_insert on requirements for insert with check (auth_role() = 'admin');
create policy req_update on requirements for update using (auth_role() in ('admin','manager')) with check (auth_role() in ('admin','manager'));
create policy req_delete on requirements for delete using (auth_role() = 'admin');
...
-- ---------- PER-REQUIREMENT DAILY ACTIVITY ----------
...
-- daily_metrics: any signed-in user reads (form needs them); admin writes.
...
-- recruiter_goals: read own/admin/manager-of-division; admins write via RLS, managers via service-role action.
create policy rg_select on recruiter_goals for select using (auth_role()='admin' or recruiter_id = auth.uid() or (auth_role()='manager' and recruiter_id in (select id from profiles where division_id = auth_division())));
...
-- ---------- MESSAGES (recipient reads/marks own; managers & admins send) ----------
create policy messages_select on messages for select using (
  recipient_id = auth.uid() or sender_id = auth.uid() or auth_role() in ('admin','manager')
);
...
-- ---------- BADGES (authenticated read; admin write) + AWARDS ----------
...
create policy rb_admin_all on recruiter_badges for all using (auth_role()='admin') with check (auth_role()='admin');
-- award inserts happen via the service role (badge engine / cron).

-- NOTE: Managers now oversee ALL divisions. Manager dashboard/requirements pages and their
-- server actions run through the service-role client (gated by role in code), so cross-division
-- reads/allocations work without widening these RLS policies. Recruiter/admin flows are unchanged.


-- =============================================================
-- STORAGE (profile + candidate photos)
-- Public buckets; signed-in users upload, anyone can view.
-- =============================================================
insert into storage.buckets (id, name, public) values ('avatars', 'avatars', true) on conflict (id) do update set public = true;
insert into storage.buckets (id, name, public) values ('candidates', 'candidates', true) on conflict (id) do update set public = true;

drop policy if exists "media_public_read" on storage.objects;
drop policy if exists "media_auth_insert" on storage.objects;
drop policy if exists "media_auth_update" on storage.objects;
drop policy if exists "media_auth_delete" on storage.objects;
create policy "media_public_read" on storage.objects for select using (bucket_id in ('avatars','candidates'));
create policy "media_auth_insert" on storage.objects for insert with check (bucket_id in ('avatars','candidates') and auth.role()='authenticated');
create policy "media_auth_update" on storage.objects for update using (bucket_id in ('avatars','candidates') and auth.role()='authenticated');
create policy "media_auth_delete" on storage.objects for delete using (bucket_id in ('avatars','candidates') and auth.role()='authenticated');
...
exception when others then
  raise notice 'Demo account seeding skipped (%). Create the 3 demo users via Supabase Authentication instead, then set their roles.', sqlerrm;
end $$;

-- Map demo people into profile_divisions. The recruiter gets BOTH US IT and India IT
-- so the multi-division allocation flow is demonstrable out of the box.
insert into profile_divisions (profile_id, division_id)
  select p.id, p.division_id from public.profiles p where p.division_id is not null
  on conflict do nothing;
insert into profile_divisions (profile_id, division_id)
  select p.id, d.id from public.profiles p
  cross join public.divisions d
  where p.email = 'recruiter@demo.com' and d.code in ('US_IT','IND_IT')
  on conflict do nothing;
...
-- ---------- PERFORMANCE INDEXES (v8) — speed up dashboard reads ----------
