...
// Same link the 3 AM IST cron emails; admins can copy it on demand.
export async function createDailyLink(recruiterId: string) {
  ...
  if (res.ok) await logAudit(me.id, "daily_link.create", "profiles", recruiterId, { for_date: res.forDate });
  return res;
}
